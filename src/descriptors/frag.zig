...
pub const FragmentDescriptor = struct {
    ...
    pub fn getCode(ptr: *anyopaque, init: std.process.Init, data: Descriptor.Data) anyerror![]u8 {
        ...
        defer { for (include_contents.items) |c| gpa.free(c); include_contents.deinit(gpa); }
        var visited = std.StringHashMap(void).init(gpa);
        defer visited.deinit();

        var include_queue = std.ArrayList([]u8).empty;
        defer { for (include_queue.items) |s| gpa.free(s); include_queue.deinit(gpa); }
        ...
        for (include_contents.items) |c| { try combined.appendSlice(gpa, c); try combined.append(gpa, '\n'); }
        {
        ...
        if (uniforms.len == 0) {
            ...
            for (uniforms) |u| {
                const zig_type_raw: []u8 = blk: {
                    ...
                    } else if (u.is_sampler) break :blk try gpa.dupe(u8, "@import(\"gl_graphics\").Texture")
...
fn blk_has(uniforms: []common.UniformDef) bool { for (uniforms) |u| if (u.kind == .block) return true; return false; }
