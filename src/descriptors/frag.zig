...
pub const FragmentDescriptor = struct {
    ...
    pub fn getCode(ptr: *anyopaque, init: std.process.Init, data: Descriptor.Data) anyerror![]u8 {
        ...
        try inner.appendSlice(gpa, ";\n");
        try inner.appendSlice(gpa, "        id = @import(\"gl_graphics\").compileShaderSource(.fragment_shader, src);\n");
        try inner.appendSlice(gpa, "        _initialized = true;\n");
        try inner.appendSlice(gpa, "        return id;\n");
        try inner.appendSlice(gpa, "    }\n\n");
        try inner.appendSlice(gpa, "    pub fn dispose() void { @import(\"gl_graphics\").disposeShader(&id, &_initialized); }\n\n");
