...
pub const FragmentDescriptor = struct {
    ...
    pub fn getCode(ptr: *anyopaque, init: std.process.Init, data: Descriptor.Data) anyerror![]u8 {
        ...
        try inner.appendSlice(gpa, "                } else if (loc != -1 and T == f32) gl.uniforms.uniform1f(loc, value) else if (loc != -1 and T == i32) gl.uniforms.uniform1i(loc, value) else if (loc != -1 and T == u32) gl.uniforms.uniform1ui(loc, value) else if (loc != -1 and T == bool) gl.uniforms.uniform1i(loc, if (value) 1 else 0) else if (@typeInfo(T) == .@\"struct\") { UniformUpload.flatten(self._program, name, T, value, gl); } else {}\n");
        try inner.appendSlice(gpa, "                }\n");
        try inner.appendSlice(gpa, "            }\n");
